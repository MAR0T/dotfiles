...
To see topology of your CPU to check which core has hyper-threading, check
```sh
lstopo
```

To check frequency of each core run
```sh
lscpu -e
```

## Pass selected cores to your VM
```xml
...
```
Let one core perform only IO operations and another for emulator to prevent intermittent slowdowns of whole VM.

Final XML of my VM:
```xml
<domain xmlns:qemu="http://libvirt.org/schemas/domain/qemu/1.0" type="kvm">
  <name>Win10_PCI_passthrough</name>
  <uuid>d7ce7781-29c1-40d0-be33-04d8c8699a17</uuid>
  <metadata>
  ...
  <os firmware="efi">
    ...
    <nvram template="/usr/share/edk2/x64/OVMF_VARS.4m.fd">/var/lib/libvirt/qemu/nvram/Win10_PCI_passthrough_VARS.fd</nvram>
  </os>
  <features>
    <acpi/>
    <apic/>
  ...
  <qemu:commandline>
    ...
    <qemu:arg value="input-linux,id=mouse1,evdev=/dev/input/by-id/usb-MIONIX_MIONIX_CASTOR_PRO_Gaming_Mouse-event-mouse"/>
    <qemu:arg value="-object"/>
    <qemu:arg value="input-linux,id=keyboard1,evdev=/dev/input/by-id/usb-ROYUAN_AKKO_Multi-modes_Keyboard-B-event-kbd,grab_all=on,repeat=on"/>
